...
<Callout type="info">
For simple object-based inserts, use `create()`. For complex SQL operations, use `insert()`.
</Callout>
...
<Callout type="info">
The `insert()` method uses raw SQL, unlike `create()` which accepts objects. For complex SQL operations, use `insert()`.
</Callout>
