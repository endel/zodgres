---
title: Collections
description: Create type-safe collections with Zod schemas and automatic migrations
---

Collections in `zodgres` are type-safe database tables defined using Zod schemas. They automatically handle table creation, migrations, and provide full TypeScript support.

## Example

Create a collection by defining a Zod schema:

```typescript
import { connect, z } from 'zodgres';

const db = await connect('postgres://localhost:5432/mydb');

const users = await db.collection('users', {
  id: z.number().optional(),        // Auto-incrementing primary key
  name: z.string().max(100),        // Required string with max length
  email: z.string().email(),        // Required email validation
  age: z.number().min(0).optional(), // Optional positive number
});
```

## Schema Definition

### Data Types

Zodgres maps Zod types to PostgreSQL column types:

| Zod Type | PostgreSQL Type | Example |
|----------|----------------|---------|
| `z.string()` | `TEXT` | `z.string()` |
| `z.string().max()` | `CHARACTER VARYING` | `z.string().max(255)` |
| `z.number()` | `NUMERIC` | `z.number().int()` |
| `z.boolean()` | `BOOLEAN` | `z.boolean()` |
| `z.date()` | `TIMESTAMP` | `z.date()` |
| `z.enum()` | `ENUM` | `z.enum(['active', 'inactive'])` |
| `z.array()` | `JSONB` | `z.array(z.string())` |
| `z.object()` | `JSONB` | `z.object({ key: z.string() })` |

### Primary Keys

Define auto-incrementing primary keys with optional numbers:

```typescript
const products = await db.collection('products', {
  id: z.number().optional(), // Auto-incrementing primary key
  name: z.string(),
  price: z.number().positive(),
});
```

<Callout type="info">
  **Convention:** Use `id: z.number().optional()` for auto-incrementing primary keys.
</Callout>

### String Constraints

Apply various string constraints:

```typescript
const articles = await db.collection('articles', {
  id: z.number().optional(),
  title: z.string().max(200),           // Max length
  slug: z.string().min(3).max(50),      // Min and max length
  content: z.string(),                   // Unlimited text
  status: z.enum(['draft', 'published']), // Enum values
});
```

### Number Constraints

Define numeric constraints and types:

```typescript
const orders = await db.collection('orders', {
  id: z.number().optional(),
  total: z.number().positive(),          // Must be positive
  quantity: z.number().int().min(1),     // Integer >= 1
  discount: z.number().min(0).max(1),    // Between 0 and 1
  rating: z.number().min(1).max(5).optional(), // Optional rating
});
```

### Optional Fields

Make fields optional with `.optional()`:

```typescript
const profiles = await db.collection('profiles', {
  id: z.number().optional(),
  user_id: z.number(),                   // Required
  bio: z.string().optional(),            // Optional
  avatar_url: z.string().url().optional(), // Optional URL
  birth_date: z.date().optional(),       // Optional date
});
```

## Advanced Schema Features

### Unique Constraints

Create unique constraints using the custom `.unique()` method:

```typescript
const users = await db.collection('users', {
  id: z.number().optional(),
  email: z.string().email().unique(),    // Unique email
  username: z.string().min(3).unique(),  // Unique username
  name: z.string(),
});
```

### JSON Fields

Store complex data as JSON:

```typescript
const settings = await db.collection('user_settings', {
  id: z.number().optional(),
  user_id: z.number(),
  preferences: z.object({
    theme: z.enum(['light', 'dark']),
    notifications: z.boolean(),
    language: z.string(),
  }),
  tags: z.array(z.string()),             // Array of strings
});
```

### Date and Time

Handle timestamps and dates:

```typescript
const events = await db.collection('events', {
  id: z.number().optional(),
  name: z.string(),
  start_time: z.date(),                  // Timestamp
  end_time: z.date(),
  created_at: z.date().optional(),       // Auto-managed timestamps
  updated_at: z.date().optional(),
});
```

## Collection Parameters

Pass additional parameters to customize collection behavior:

```typescript
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
}, {
  strict: true,    // Reject unknown properties
  strip: false,    // Don't strip unknown properties
});
```

## Type Inference

Zodgres provides full TypeScript type inference:

```typescript
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
  age: z.number().optional(),
});

// TypeScript infers the correct types
const user = await users.create({
  name: 'John',  // ✅ Required
  age: 30,       // ✅ Optional
  // email: 'test' // ❌ TypeScript error - not in schema
});

// Return type is inferred as:
// { id: number, name: string, age: number | null }
```

## Table Management

### Automatic Migration

Collections automatically create tables when first accessed:

```typescript
// Table is created automatically on first collection access
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
});
```

### Manual Migration

Force table creation or updates:

```typescript
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
  email: z.string().email(),
});

// Manually trigger migration
await users.migrate();
```

### Drop Tables

Remove tables when needed:

```typescript
// Drop the entire table
await users.drop();
```

<Callout type="warning">
  **Warning:** `drop()` permanently deletes the table and all its data. Use with caution.
</Callout>

## Schema Evolution

### Adding Fields

Add new optional fields to existing schemas:

```typescript
// Original schema
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
});

// Later, add optional fields
const usersV2 = await db.collection('users', {
  id: z.number().optional(),
  name: z.string(),
  email: z.string().email().optional(), // New optional field
  created_at: z.date().optional(),       // New optional field
});
```

### Schema Validation

Zodgres validates data against your schema on every operation:

```typescript
const users = await db.collection('users', {
  id: z.number().optional(),
  name: z.string().max(50),
  age: z.number().min(0).max(120),
});

// ❌ This will throw a validation error
await users.create({
  name: 'A'.repeat(100), // Too long
  age: -5,               // Negative age
});
```

## Best Practices

### Naming Conventions

- Use snake_case for field names to match PostgreSQL conventions
- Use plural names for collection/table names
- Be explicit with constraints

```typescript
const user_profiles = await db.collection('user_profiles', {
  id: z.number().optional(),
  user_id: z.number(),
  first_name: z.string().max(50),
  last_name: z.string().max(50),
  created_at: z.date().optional(),
  updated_at: z.date().optional(),
});
```

### Schema Organization

Organize schemas in separate files for maintainability:

```typescript
// schemas/user.ts
export const userSchema = {
  id: z.number().optional(),
  name: z.string().max(100),
  email: z.string().email().unique(),
  created_at: z.date().optional(),
};

// main.ts
import { userSchema } from './schemas/user';
const users = await db.collection('users', userSchema);
```

## Next Steps

Now that you have collections defined, learn how to [query your data](/querying-collections).